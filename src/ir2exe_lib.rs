...
fn build_llc_compile_cmd(
    ...
    if is_windows {
        cmd.arg("-mtriple=x86_64-w64-mingw32");
    } else if is_darwin {
        cmd.arg("-mtriple=x86_64-apple-darwin");
    } else if is_wasm {
        cmd.arg("-mtriple=wasm32-unknown-unknown");
    } else {
        // Linux/Unix: 必须显式设置目标三元组以覆盖 IR 文件中嵌入的目标
        // 否则 llc 会使用 IR 中的目标（可能是 Windows），导致目标文件格式不匹配
        cmd.arg("-mtriple=x86_64-unknown-linux-gnu");
    }

    // CPU 指令集
    if let Some(ref march) = options.march {
        cmd.arg(format!("-mcpu={}", march));
    }
...
fn compile_object_with_llc(
    ...
    let mut cmd = build_llc_compile_cmd(llc_exe, input_file, output_file, options);
