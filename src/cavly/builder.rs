use crate::cavly::config::{BinTarget, CavlyConfig, ProjectType};
use crate::cavly::workspace::{ResolvedDependency, WorkspaceResolver, topological_sort};
use crate::cavly::{TARGET_DIR, ensure_dir};
...
impl Builder {
    ...
    fn install_library(&self, output_path: &Path) -> Result<()> {
        ...
        if output_path != install_path {
            std::fs::copy(output_path, &install_path).with_context(|| {
                format!(
                    "安装库文件失败: {} -> {}",
                    output_path.display(),
    ...
    fn generate_header(&self, lib_dir: &Path) -> Result<()> {
        ...
        let mut header_content = format!(
            r#"/* Cavvy Library Header - Auto Generated */
/* Package: {} */
/* Version: {} */
#ifndef {}_H
#define {}_H
...
"#,
            self.config.package.name, self.config.package.version, guard_name, guard_name
        );
...
pub fn quick_build(project_root: &Path, verbose: bool) -> Result<Vec<PathBuf>> {
    ...
    let mut builder = Builder::new(project_root.to_path_buf(), config).verbose(verbose);
...
mod tests {
    use super::*;
    use crate::cavly::config::{BuildConfig, PackageConfig};
    ...
    fn test_builder_verbose() {
        let temp = TempDir::new().unwrap();
        let config = create_test_config();
        let builder = Builder::new(temp.path().to_path_buf(), config).verbose(true);
