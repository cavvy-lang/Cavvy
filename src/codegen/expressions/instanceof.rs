...
impl IRGenerator {
    ...
    pub fn generate_instanceof_expression(
        ...
        self.emit_line(&format!(
            "  br i1 {}, label %{}, label %{}",
            is_null, null_label, check_label
        ));
        ...
        self.emit_line(&format!(
            "  {} = bitcast {} {} to i32*",
            type_id_ptr, expr_type, expr_val
        ));

        let actual_type_id = self.new_temp();
        self.emit_line(&format!(
            "  {} = load i32, i32* {}",
            actual_type_id, type_id_ptr
        ));
