...
impl IRGenerator {
    ...
    /// # Returns
    /// (目标类型, 提升后的左值, 提升后的右值)
    pub fn promote_integer_operands(
        &mut self,
        left_type: &str,
        left_val: &str,
        right_type: &str,
        ...
        let target_type = if left_bits >= right_bits {
            left_type
        } else {
            right_type
        };
        ...
        let promoted_left = if left_type != target_type {
            ...
            self.emit_line(&format!(
                "  {} = sext {} {} to {}",
                temp, left_type, left_val, target_type
            ));
            temp
        } else {
            left_val.to_string()
        ...
        let promoted_right = if right_type != target_type {
            ...
            self.emit_line(&format!(
                "  {} = sext {} {} to {}",
                temp, right_type, right_val, target_type
            ));
            temp
        } else {
            right_val.to_string()
    ...
    pub fn promote_mixed_operands(
        ...
        if left_is_int && right_is_float {
            // 整数 + 浮点数：将整数转换为浮点数
            let promoted_type = if right_type == "double" {
                "double"
            } else {
                "float"
            };
            ...
            Some((
                promoted_type.to_string(),
                converted_left,
                right_val.to_string(),
            ))
        } else if left_is_float && right_is_int {
            // 浮点数 + 整数：将整数转换为浮点数
            let promoted_type = if left_type == "double" {
                "double"
            } else {
                "float"
            };
    ...
    pub fn get_lvalue_info_with_param_flag(
        ...
        match expr {
            Expr::Identifier(name) => {
                ...
                // 优先使用作用域管理器获取变量类型
                let (var_type, llvm_name, is_param) = if let Some(scope_type) =
                    self.scope_manager.get_var_type(name_str)
                {
                    let llvm_name = self
                        .scope_manager
                        .get_llvm_name(name_str)
    ...
    pub fn generate_division_by_zero_check(&mut self, val_type: &str, val: &str) -> cayResult<()> {
        ...
        self.emit_line(&format!(
            "  br i1 {}, label %{}, label %{}",
            is_zero, error_label, continue_label
        ));
    ...
    pub fn llvm_type_to_signature_with_varargs(
        &self,
        llvm_type: &str,
        ...
        let class_name_opt: Option<String> = if let Expr::Identifier(name) = member.object.as_ref()
        {
            ...
            if name_str == "this" {
                ...
                if var_class.is_some() {
                    ...
                    let has_static_fields = self
                        .static_field_map
                        .keys()
                        .any(|k| k.starts_with(&static_key_prefix));
    ...
    fn get_nested_field_pointer_recursive(
        &mut self,
        member: &MemberAccessExpr,
        is_root: bool,
    ) -> cayResult<(String, String)> {
        ...
        let (obj_ptr, obj_class_name) = match member.object.as_ref() {
            ...
            Expr::MemberAccess(nested_member) => {
                ...
                // 同时获取字段信息以确定对象类型
                let (nested_type, nested_ptr, field_info_opt) =
                    self.get_nested_field_pointer_recursive_with_info(nested_member, true)?;
                ...
                self.emit_line(&format!(
                    ...
                    nested_type,
                    nested_ptr,
                    self.get_type_align(&nested_type)
                ));
    ...
    fn get_nested_field_pointer_recursive_with_info(
        &mut self,
        member: &MemberAccessExpr,
        is_root: bool,
    ) -> cayResult<(
