"""
Cluster 39 - lambda_expressions: Lambda模糊测试
"""
...
def generate(cluster_id,cluster_name,cluster_dir,fuzz_dir):
    ...
    for fi in range(100):
        ...
        for mi in range(random.randint(8,12)):
            mn=unique_method_name("test_lambda",mi)
            a=random.randint(1,100); bv=random.randint(1,100)
            body=[f"int a={a}; int b={bv};",
                f"int c = a + b;",
                'print(\\\"lambda=\\\"); println(c);']
            body+=['println("OK");']
