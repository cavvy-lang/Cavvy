"""
Cluster 19 - while_loop: while循环模糊测试
"""
...
def generate(cluster_id,cluster_name,cluster_dir,fuzz_dir):
    ...
    for fi in range(100):
        ...
        for mi in range(random.randint(8,14)):
            mn=unique_method_name("test_while",mi)
            n=random.randint(1,10)
            body=[f"int i = 0;",f"while(i < {n}){{println(i); i=i+1;}}"]
            n2=random.randint(1,8)
            body+=[f"int j = {n2};",f"while(j > 0){{println(j); j=j-1;}}"]
            body+=['println("OK");']
