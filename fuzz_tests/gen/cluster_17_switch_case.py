"""
Cluster 17 - switch_case: switch语句模糊测试
"""
...
def generate(cluster_id,cluster_name,cluster_dir,fuzz_dir):
    ...
    for fi in range(100):
        ...
        for mi in range(random.randint(8,14)):
            mn=unique_method_name("test_sw",mi)
            v=random.randint(0,9)
            body=[f"int x = {v};", "switch(x){"]
            cases=set()
            for ci in range(random.randint(2,5)):
                cv=random.randint(0,9); cases.add(cv)
            for cv in sorted(cases):
                body+=["  case %d:"%cv,'    println("%d");'%cv,"    break;"]
            body+=["  default:",'    println("default");',"    break;","}"]
            body+=['println("OK");']
