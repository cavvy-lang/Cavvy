"""
Cluster 21 - enhanced_for: 增强for循环模糊测试
"""
...
def generate(cluster_id,cluster_name,cluster_dir,fuzz_dir):
    ...
    for fi in range(100):
        ...
        for mi in range(random.randint(8,14)):
            mn=unique_method_name("test_ef",mi)
            n=random.randint(1,8)
            body=[f"int[] arr = new int[{n}];",
                f"int len = arr.length;",
                f"for(int i=0;i<len;i=i+1){{arr[i]=i*10+{random.randint(1,10)};}}",
                "for(int i=0;i<len;i=i+1){int v=arr[i]; println(v);}"]
            body+=['println("OK");']
