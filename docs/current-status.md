...
| 接口声明与实现 | 可用 | `tests/interface_tests.rs` 覆盖接口变量、接口参数、多接口和继承组合 |
...
接口类型调用已经能覆盖常见的单实现调用、参数传递和返回值场景，但它和“基类类型 vtable 分发”不是同一条代码路径。当前实测中，两个不同实现类通过同一个接口类型调用同名方法时，仍不能依赖运行时类型分派：
...
当前 release 编译器对这个探针输出 `Dog`、`Dog`，而不是 `Dog`、`Cat`。因此文档示例可以使用接口类型和单实现接口调用，但不要把接口值的多实现运行时分派写成已完成语义。类继承的 vtable 动态分发已经通过单独测试覆盖。
