...
| 特性 | 状态 | 说明 |
|---|---|---|
| **泛型单态化**（代码生成） | ❌ 未实现 | 泛型解析可用，但单态化代码生成未实现 |
| **Lambda 闭包捕获完整实现** | ⚠️ 部分实现 | 闭包捕获环境变量的完整功能尚未完成 |
| **private 访问控制强制执行** | ❌ 未强制执行 | 语义分析检查已实现，但编译器不阻止跨类访问 |
| **数组初始化语法** `new Type[] { 1, 2, 3 }` | ❌ 不支持 | 需先声明大小再逐个赋值 |
| **inline IR（__ir { }）** | ✅ 已实现 | 参见 `src/ir/inline_ir.rs` |
