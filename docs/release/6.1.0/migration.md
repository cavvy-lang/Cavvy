...
- 将 `MmapResult`、`FileResult` 等专用结果逐步统一为 `Result<T, E>`。（已于 6.2.0 完成：`MmapResult`/`FileResult` 已删除，`File.openResult`/`File.existsResult`/`Mmap.mapReadOnly`/`Mmap.mapReadWrite` 均返回 `Result<_, IOError>`。）
